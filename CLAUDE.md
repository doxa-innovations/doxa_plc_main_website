...
11. **Payload ignores the auth cookie on a navigation that arrives cross-site.** With `csrf` configured, a request with no `Origin` header only has its `payload-token` read if `Sec-Fetch-Site` is `same-origin`, `same-site` or `none` (`node_modules/payload/dist/auth/extractJWT.js`). A 307 from a route reached via another site (e.g. the Google callback) keeps the chain cross-site, so the next page sees a valid session as signed out. That is why `app/olympus/login/google/callback/route.ts` ends with a small same-origin handoff page instead of `redirect("/olympus")`. It also means `curl` against /olympus needs `-H "Sec-Fetch-Site: same-origin"` to be treated as signed in.
...
- **Google sign-in for /olympus:** `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. Unset hides the button; the password form always works. The allowlist is `ALLOWED_GOOGLE_EMAILS` in `lib/google-auth.ts`, not an env var. The flow ends by minting an ordinary Payload session and `payload-token` (mirroring Payload's login operation), so `proxy.ts` and `requireUser()` need nothing Google-specific. The redirect URI is `${NEXT_PUBLIC_SITE_URL}/olympus/login/google/callback` and must match Google Cloud Console exactly, www included; locally that means `NEXT_PUBLIC_SITE_URL=http://localhost:3000`.
